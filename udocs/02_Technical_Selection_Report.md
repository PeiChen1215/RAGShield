...
# 关键：创建时指定 hnsw:space="cosine"，ChromaDB 直接返回余弦距离
chroma_client = chromadb.PersistentClient(path="./data/chroma_db")
collection = chroma_client.get_or_create_collection(
    name="ragshield_kb",
    metadata={"hnsw:space": "cosine"}  # 直接返回余弦距离
)
# 查询时返回的 distance 已经是余弦距离（1 - cosine_similarity）
# 转换为相似度：similarity = 1 - distance
```<br><br>**为什么选 cosine space（Q11 答案）**：<br>1. BGE-M3 默认 L2 归一化，可用 `cosine_sim = 1 - l2_distance^2 / 2` 转换，但多一步计算<br>2. ChromaDB 原生支持 cosine 空间索引，直接返回 `distance = 1 - cosine_sim`，无需手动转换<br>3. Layer2 的方差计算、reranker 评分、阈值设定全部基于 0~1 余弦相似度语义，统一不混淆<br>4. 实现成本：仅需在 `get_or_create_collection` 时传入 `metadata`，无额外代码 |
...
| **融合策略（Q12 答案）** | 采用**混合法**：先硬阈值判定是否异常，异常者再根据异常程度映射到连续分数：<br><br>```python
...
iso_labels = iso.fit_predict(embeddings)  # -1=异常, 1=正常
lof_labels = lof.fit_predict(embeddings)   # -1=异常, 1=正常

# Step 2: 硬阈值判定
is_anomaly = (iso_labels == -1) & (lof_labels == -1)

# Step 3: 异常者映射到连续分数 (0.5~0.7)
# 使用 IF 的 decision_function（负值越大越异常）
iso_raw = iso.decision_function(embeddings)  # 范围约 -0.5 ~ 0.5
iso_norm = 1 - (iso_raw - iso_raw.min()) / (iso_raw.max() - iso_raw.min())  # 反转归一化: 异常=高分

# 异常文档的 base_score = 0.5 + 0.2 * iso_norm
# 正常文档的 base_score = 0.05（极低背景分）
base_scores = np.where(is_anomaly, 0.5 + 0.2 * iso_norm, 0.05)
```<br><br>**为什么选混合法（Q12 答案）**：<br>1. 离散判定（是否异常）用于展示和可疑文档列表<br>2. 连续分数（0.05~0.7）用于风险融合，与 Layer2/Layer3 的连续评分兼容<br>3. IF 的 `decision_function` 负值越大越异常，归一化后异常者自然映射到 0.5~0.7<br>4. 正常文档给 0.05 极低背景分，不影响融合但保持接口一致性 |
...
| **预构建镜像策略（比赛现场零依赖，Q8 答案）** | 镜像拆分为 **ragshield-core（必装）+ ragshield-llm（可选）**，避免单镜像过大：<br><br>**ragshield-core.tar**（必装，~1.7-3.2GB）：<br>- bge-small-zh-v1.5（~500MB）或 BGE-M3（~2GB，GPU 版）<br>- bge-reranker（~1GB）<br>- uer/chinanli（~110MB）<br>- HanLP（~100MB）<br>- 代码 + 依赖 + 测试数据<br><br>**ragshield-llm.tar**（可选 fallback，~8GB）：<br>- Qwen2.5-7B-Instruct INT4（仅当现场无网络时使用）<br><br>现场策略：<br>1. 优先加载 ragshield-core + Kimi API（网络正常时）<br>2. 网络不可用时，再加载 ragshield-llm 作为本地 LLM fallback<br>3. CPU 降级版镜像（bge-small 替代 BGE-M3）压缩到 ~1.7GB，U 盘拷贝更可行<br><br>构建命令：<br>```bash
# GPU 版
docker build -t ragshield:gpu --build-arg MODEL_PROFILE=gpu .
docker save ragshield:gpu > ragshield-gpu.tar  # ~3.2GB

# CPU 版
docker build -t ragshield:cpu --build-arg MODEL_PROFILE=cpu .
docker save ragshield:cpu > ragshield-cpu.tar  # ~1.7GB

# LLM fallback（可选）
docker build -t ragshield:llm -f Dockerfile.llm .
docker save ragshield:llm > ragshield-llm.tar  # ~8GB
``` |
