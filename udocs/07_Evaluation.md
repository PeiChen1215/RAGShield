...
| 类别 | 数量 | 来源 | 说明 |
|------|------|------|------|
| **攻击查询** | **27 条** | 6 类型 × 3-7 条/类型 | tamper×7 + injection×7 + poisoning×4 + leak×3 + privilege×3 + social×3 |
| **正常查询** | **20 条** | 自建 | 覆盖 HR/IT/产品/公司制度/研发/运维/法务/市场/客服等主题 |
| **正常文档** | 100 篇 | 自建 | 10 类别：行政/IT/HR/产品/财务/研发/运维/法务/市场/客服 |
| **攻击文档** | 25 篇 | 自建 | 6 类型：fact_tamper×6 / instruction_injection×6 / data_poisoning×4 / prompt_leak×3 / privilege_escalation×3 / social_engineering×3 |
...
- [x] 检测率 ≥ 90%（6 类攻击，27 条攻击查询）→ **实际 100%**
- [x] 误报率 ≤ 5%（20 条正常查询中 ≤1 条误判）→ **实际 5%（已修复，当前代码 0%）**
- [x] 3 组演示用例全部可跑通（见 4.3 节）
- [x] 冒烟测试 100% 通过（pytest 7/7）
- [ ] 检测延迟 P50 < 100ms → **实际 ~12.5s（LLM API 调用为主要瓶颈，已标注为"生成延迟"）**
