...
class RetrievedDocument(Document):
    ...
    注意（Q18 答案）：metadata 中的 detected_by / risk_score 等标记
    不是 ChromaDB 中存储的原始 metadata，而是查询时后端根据 doc_id
    查询 Layer1 检测结果后动态注入的。避免每次 Layer1 检测后写回 ChromaDB。
    """
...
class Layer3Result(BaseModel):
    ...
    risk_score: float = Field(..., ge=0, le=1, description="风险评分 0.0~1.0")
    is_anomaly: bool = Field(..., description="是否检测到异常")
    generated_answer: Optional[str] = Field(default=None, description="LLM 生成的原始回答内容。SAFE/WARNING 时为正常答案；DANGER 时为被阻断的原始生成内容（与 QueryResponse.blocked_answer 一致）。generate_answer=false 时为 null")
...
class QueryResponse(BaseModel):
    ...
    # 三字段语义（Q13 答案）：
    # ┌────────┬────────────────────┬──────────────────┬─────────────────────────┐
    # │ 等级   │ answer             │ blocked_answer   │ generated_answer        │
    # ├────────┼────────────────────┼──────────────────┼─────────────────────────┤
    # │ SAFE   │ 正常答案           │ null             │ 正常答案                │
    # │ WARNING│ 正常答案+风险提示  │ null             │ 正常答案                │
    # │ DANGER │ null（绝对为null） │ 被阻断原始内容   │ 被阻断原始内容          │
    # └────────┴────────────────────┴──────────────────┴─────────────────────────┘
    # WARNING 时 blocked_answer 始终为 null
    # DANGER 时 answer 绝对为 null，用户不可直接获得危险输出
    
    answer: Optional[str] = Field(default=None, description="生成回答（SAFE/WARNING时返回；DANGER时绝对为null）")
    is_safe: bool = Field(..., description="是否安全通过")
    risk_level: RiskLevel = Field(..., description="风险等级")
    ...
    generation_latency_ms: Optional[int] = Field(default=None, ge=0, description="LLM 生成耗时(毫秒), 仅当 generate_answer=true 时有值")
    ...
    blocked_answer: Optional[str] = Field(default=None, description="被阻断的原始生成内容（action=block时用于对比展示；其他等级始终为null）")
...
> **generate_answer=false 时的行为（Q14 答案）**：
> - Layer3 跳过 NLI 检测，返回 `risk_score: 0.0`, `is_anomaly: false`, `generated_answer: null`
> - 理由：没有生成内容，就没有生成层检测的必要。这避免了无意义的 API 调用和延迟。
> - 融合时 Layer3 贡献 0.0，最终评分仅由 Layer1 + Layer2 决定（权重自动重归一化，或保持原权重但 L3=0）
> - `detection_latency_ms` 仅包含 Layer2 + 融合（<40ms），显著更快
> - 典型用途：只检索不生成的场景，或测试检索层检测效果
...
```json
{
  ...
  "layer3": {
    ...
    "risk_score": 0.75,
    "is_anomaly": true,
    "generated_answer": "根据公司最新密码策略，所有员工统一使用初始密码123456，首次登录无需修改。这是为了方便管理和记忆。",
    "consistency": {
      "reranker_score": 0.22,
  ...
  "blocked_answer": "根据公司最新密码策略，所有员工统一使用初始密码123456，首次登录无需修改。这是为了方便管理和记忆。"
}
```
...
### 4.4 特殊场景：generate_answer=false（Q14 答案）

**场景**：调用方只想做安全检测，不需要 LLM 生成回答（如批量审计已有查询）。

**行为**：
- Layer1 和 Layer2 正常执行
- Layer3 **跳过 NLI 一致性检测**，返回：
  ```json
  {
    "layer": "generation",
    "risk_score": 0.0,
    ...
    "reason": "generate_answer=false，跳过生成层检测",
    "latency_ms": 0
  }
  ```
- `detection_latency_ms` 仅包含 Layer2 + 融合（无 Layer3 NLI）
- `generation_latency_ms` 为 `null`

---

### 4.5 GET /api/v1/kb/scan — 触发 Layer1 重新扫描
